...
class Gemm_sf(theano.Op):
    ...
    def grad(self, inputs, outputs):
        ...
        xgrad = T.dot(gzo, b.T)
        xgrad = (xgrad.reshape((am.shape[0], am.shape[1], self.blocksize)) * am.dimshuffle(0, 1, 'x')).reshape(
            xgrad.shape)
...
if __name__ == "__main__":
    ...
    zt = sparse_theano_ss(A.T, am, B, bm, c, bs)  # A를 전치하여 사용
    zfs = Gemm_fs(bs)(A.T, B, bm, c)  # A를 전치하여 사용
    zfst = sparse_theano_fs(A.T, B, bm, c, bs)  # A를 전치하여 사용
    ...
    am = numpy.random.uniform(0, 1, (n, int(p / bs))) < 0.105
    bm = numpy.random.uniform(0, 1, (n, int(m / bs))) < 0.105
