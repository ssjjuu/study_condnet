...
def model_cifar_1(policies, sample_probabilities, layer_masks, activations,
    ...
    model = StackModel([
        ...
        ConvLayer((nfilters, 3, 3, 3), relu, mode='valid', normalize=False, stride=(2,2)), # 12,12
        ...
        Maxpool((2,2)), # 6,6
        #dropout(0.5),
        ConvLayer((10, nfilters, 3, 3), relu, mode='valid', normalize=False), # 2,2
...
def main(exp_params):
    ...
    if "condnet_min_prob" in exp_params:
        condnet_min_prob.set_value(np.float32(exp_params["condnet_min_prob"]))
